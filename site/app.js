...
const DOCS_SEARCH_INDEX = [
  ...
  },
  {
    ...
    summary: "Frozen v1 specification for 14 MCP tools, 7 resources, 6 prompts, and JSON schemas, plus the additive go_context tool in v1.2.1.",
...
function generateInstallCommandPayload(params = {}) {
  ...
  if (method === 'brew') {
    ...
    command = 'curl -fsSL https://raw.githubusercontent.com/agentic-mcps/go/v1.2.1/scripts/install.sh | bash -s -- 1.2.1';
    instructions = 'Downloads and installs the latest pinned v1.2.1 release binaries to /usr/local/bin or ~/.local/bin.';
  } else {
    const archiveName = `agentic-go_1.2.1_${os}_${arch}.tar.gz`;
    command = `curl -LO https://github.com/agentic-mcps/go/releases/download/v1.2.1/${archiveName}\ntar -xzf ${archiveName}\nsudo mv agentic-go agentic-go-gopls agentic-go-vet /usr/local/bin/`;
...
function initWebMCP() {
  ...
  const agentTools = [
    ...
    },
    {
      ...
      curl: "curl -fsSL https://raw.githubusercontent.com/agentic-mcps/go/v1.2.1/scripts/install.sh | bash -s -- 1.2.1",
          mcpConfig: {
            mcpServers: {
              "agentic-go": {
                command: "agentic-go",
    ...
    },
    {
      ...
      description: "Returns the current v1.2.1 surface of agentic-go: 15 tools (14 frozen v1 tools plus additive go_context), 7 resources, 1 resource template, and 6 prompts.",
      inputSchema: {
        type: "object",
        properties: {},
        additionalProperties: false
      ...
      execute: async () => {
        const catalog = {
          ...
          tools: [
            ...
            "go_audit_concurrency", "go_audit_errors", "go_context"
          ],
          resourcesCount: 7,
          resources: [
